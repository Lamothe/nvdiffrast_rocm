...
namespace HR
{
    ...
    __device__ __inline__ void add_add_carry(U32 &rlo, U32 alo, U32 blo, U32 &rhi, U32 ahi, U32 bhi)
    {
    ...
    // Helper for sign-extending 16-bit halves
    __device__ __inline__ int32_t sext_lo(int32_t a) { return (int32_t)(int16_t)a; }
    __device__ __inline__ int32_t sext_hi(int32_t a) { return (int32_t)(int16_t)(a >> 16); }
    __device__ __inline__ uint32_t zext_lo(uint32_t a) { return (uint32_t)(uint16_t)a; }
    __device__ __inline__ uint32_t zext_hi(uint32_t a) { return (uint32_t)(uint16_t)(a >> 16); }

    // S16 math (Sign extended)
    __device__ __forceinline__ int32_t add_s16lo_s16lo(uint32_t a, uint32_t b)
    {
        return (int32_t)(int16_t)(a & 0xFFFF) + (int32_t)(int16_t)(b & 0xFFFF);
    }
    __device__ __forceinline__ int32_t add_s16hi_s16lo(uint32_t a, uint32_t b)
    {
        return (int32_t)(int16_t)(a >> 16) + (int32_t)(int16_t)(b & 0xFFFF);
    }
    __device__ __inline__ S32 add_s16lo_s16hi(S32 a, S32 b) { return sext_lo(a) + sext_hi(b); }
    __device__ __inline__ S32 add_s16hi_s16hi(S32 a, S32 b) { return sext_hi(a) + sext_hi(b); }
    __device__ __forceinline__ int32_t sub_s16lo_s16lo(uint32_t a, uint32_t b)
    {
        return (int32_t)(int16_t)(a & 0xFFFF) - (int32_t)(int16_t)(b & 0xFFFF);
    }
    __device__ __inline__ S32 sub_s16hi_s16lo(S32 a, S32 b) { return sext_hi(a) - sext_lo(b); }
    __device__ __inline__ S32 sub_s16lo_s16hi(S32 a, S32 b) { return sext_lo(a) - sext_hi(b); }

    __device__ __forceinline__ int32_t sub_s16hi_s16hi(uint32_t a, uint32_t b)
    {
        return (int32_t)(int16_t)(a >> 16) - (int32_t)(int16_t)(b >> 16);
    }

    // U16 math (Zero extended)
    __device__ __inline__ S32 sub_u16lo_u16lo(U32 a, U32 b) { return (S32)(zext_lo(a) - zext_lo(b)); }
    __device__ __inline__ S32 sub_u16hi_u16lo(U32 a, U32 b) { return (S32)(zext_hi(a) - zext_lo(b)); }
    __device__ __inline__ S32 sub_u16lo_u16hi(U32 a, U32 b) { return (S32)(zext_lo(a) - zext_hi(b)); }
    __device__ __inline__ S32 sub_u16hi_u16hi(U32 a, U32 b) { return (S32)(zext_hi(a) - zext_hi(b)); }
    ...
    __device__ __inline__ U32 add_b2(U32 a, U32 b) { return ((a >> 16) & 0xFF) + b; }
    __device__ __inline__ U32 add_b3(U32 a, U32 b) { return ((a >> 24) & 0xFF) + b; }

    // Vector Multiply-Add (vmad) - Integer multiply then add
    __device__ __inline__ U32 vmad_b0(U32 a, U32 b, U32 c) { return ((a & 0xFF) * b) + c; }
    __device__ __inline__ U32 vmad_b1(U32 a, U32 b, U32 c) { return (((a >> 8) & 0xFF) * b) + c; }
    __device__ __inline__ U32 vmad_b2(U32 a, U32 b, U32 c) { return (((a >> 16) & 0xFF) * b) + c; }
    __device__ __inline__ U32 vmad_b3(U32 a, U32 b, U32 c) { return (((a >> 24) & 0xFF) * b) + c; }
    __device__ __inline__ U32 vmad_b0_b3(U32 a, U32 b, U32 c) { return ((a & 0xFF) * ((b >> 24) & 0xFF)) + c; }
    __device__ __inline__ U32 vmad_b1_b3(U32 a, U32 b, U32 c) { return (((a >> 8) & 0xFF) * ((b >> 24) & 0xFF)) + c; }
    __device__ __inline__ U32 vmad_b2_b3(U32 a, U32 b, U32 c) { return (((a >> 16) & 0xFF) * ((b >> 24) & 0xFF)) + c; }
    __device__ __inline__ U32 vmad_b3_b3(U32 a, U32 b, U32 c) { return (((a >> 24) & 0xFF) * ((b >> 24) & 0xFF)) + c; }

    // Mask math (just extraction?)
    ...
    // Min/Max/Add combos
    __device__ __forceinline__ int32_t min_min(int32_t a, int32_t b, int32_t c)
    {
        return ::min(::min(a, b), c);
    }

    __device__ __forceinline__ int32_t max_max(int32_t a, int32_t b, int32_t c)
    {
    ...
    __device__ __forceinline__ int32_t add_sub(int32_t a, int32_t b, int32_t c)
    {
        return a + b - c;
    }

    // Clamp Logic (Clamping between 0 and c, or shifting bytes)
    __device__ __forceinline__ int32_t add_clamp_0_x(int32_t a, int32_t b, int32_t x)
    {
        return ::max(0, ::min(x, a + b));
    }

    __device__ __inline__ S32 add_clamp_b0(S32 a, S32 b, S32 c)
    {
        S32 v = a + b;
        return min(max(v, 0), 0xFF); // Assuming .b0 implies clamp to byte
    }
    __device__ __inline__ S32 add_clamp_b2(S32 a, S32 b, S32 c)
    {
        S32 v = a + b;
        return min(max(v, 0), 0xFF0000); // Assuming .b2 implies clamp to 3rd byte range?
        // Note: The original PTX was `vadd.u32.s32.s32.sat %0.b2...` which is weird.
        // Standard clamping is usually safer:
        return v;
    }

    // Byte Permute (HIP has intrinsics for this)
    __device__ __forceinline__ uint32_t prmt(uint32_t a, uint32_t b, uint32_t ctrl)
    {
        // 0x5410 packs the lower 16 bits of 'a' and lower 16 bits of 'b' into a 32-bit word
        if (ctrl == 0x5410)
        {
    ...
    // Misc
    __device__ __inline__ S32 u32lo_sext(U32 a) { return (int32_t)(int16_t)a; }
    ...
    template <class T>
    __device__ __inline__ void sortShared(T *ptr, int numItems); // Assumes that numItems <= threadsInBlock. Must sync before & after the call.

    __device__ __inline__ const HRImageParams &getImageParams(const HRParams &p, int idx)
    {
        return (idx < HR_EMBED_IMAGE_PARAMS) ? p.imageParamsFirst[idx] : p.imageParamsExtra[idx - HR_EMBED_IMAGE_PARAMS];
    }
    ...
    __device__ __inline__ uint3 setupPleq(float3 values, int2 v0, int2 d1, int2 d2, F32 areaRcp)
    {
        ...
        pleq.x = (U32)(xc >> (rcpShift - (sh + HR_SUBPIXEL_LOG2)));
        pleq.y = (U32)(yc >> (rcpShift - (sh + HR_SUBPIXEL_LOG2)));

        S32 centerX = (v0.x * 2 + min_min(d1.x, d2.x, 0) + max_max(d1.x, d2.x, 0)) >> (HR_SUBPIXEL_LOG2 + 1);
        S32 centerY = (v0.y * 2 + min_min(d1.y, d2.y, 0) + max_max(d1.y, d2.y, 0)) >> (HR_SUBPIXEL_LOG2 + 1);
        S32 vcx = v0.x - (centerX << HR_SUBPIXEL_LOG2);
        S32 vcy = v0.y - (centerY << HR_SUBPIXEL_LOG2);
    ...
    __device__ __inline__ void cover8x8_setupLUT(volatile U64 *lut)
    {
        for (S32 lutIdx = threadIdx.x + blockDim.x * threadIdx.y; lutIdx < HR_COVER8X8_LUT_SIZE; lutIdx += blockDim.x * blockDim.y)
        {
    ...
    {
        F32 yinitBias = (F32)(1 << (31 - HR_MAXVIEWPORT_LOG2 - HR_SUBPIXEL_LOG2 * 2));
        F32 yinitScale = (F32)(1 << (32 - HR_SUBPIXEL_LOG2));
        F32 yincScale = 65536.0f * 65536.0f;

        S32 slctFlipY = flips << (31 - HR_FLIPBIT_FLIP_Y);
        S32 slctFlipX = flips << (31 - HR_FLIPBIT_FLIP_X);
        S32 slctSwapXY = flips << (31 - HR_FLIPBIT_SWAP_XY);

        // Evaluate cross product.

        S32 t = ox * dy - oy * dx;
        F32 det = (F32)slct(t, t - dy * (7 << HR_SUBPIXEL_LOG2), slctFlipX);
        if (flips >= (1 << HR_FLIPBIT_COMPL))
            det = -det;
    ...
    __device__ __inline__ U64 cover8x8_lookupMask(S64 yinit, U32 yinc, U32 flips, volatile const U64 *lut)
    {
        ...
        add_add_carry(yfrac, yfrac, yinc, shape, shape, shape);
        add_add_carry(yfrac, yfrac, yinc, shape, shape, shape);
        int oct = flips & ((1 << HR_FLIPBIT_FLIP_X) | (1 << HR_FLIPBIT_SWAP_XY));
        ...
        return (flips >= (1 << HR_FLIPBIT_COMPL)) ? ~mask : mask;
    }
    ...
    __device__ __inline__ U64 cover8x8_conservative_noLUT(S32 ox, S32 oy, S32 dx, S32 dy)
    {
        ...
        if (dy > 0 || (dy == 0 && dx <= 0))
            curr--; // exclusive
        curr += (::abs(dx) + ::abs(dy)) << (HR_SUBPIXEL_LOG2 - 1);
        return cover8x8_generateMask_noLUT(curr, dx, dy);
    }
    ...
    __device__ __inline__ U64 cover8x8_generateMask_noLUT(S32 curr, S32 dx, S32 dy)
    {
        curr += (dx - dy) * (7 << HR_SUBPIXEL_LOG2);
        S32 stepX = dy << (HR_SUBPIXEL_LOG2 + 1);
        S32 stepYorig = -dx - dy * 7;
        S32 stepY = stepYorig << (HR_SUBPIXEL_LOG2 + 1);
    ...
    template <class T>
    __device__ __inline__ void sortShared(T *ptr, int numItems)
    {
        ...
        // FIX: Use 64-bit integer for the mask to satisfy HIP's strict type checking
        unsigned long long actMask = __ballot_sync((unsigned long long)(~0u), act);
        ...
        if (act)
        {
            ...
            for (int iter = 0; iter < range; iter += 2)
            {
                ...
                __syncwarp(actMask); // Now receives a safe 64-bit mask
