...
namespace HR
{
    ...
    struct HRTriangleData
    {
        U32 zx; // zx * sampleX + zy * sampleY + zb = lerp(HR_DEPTH_MIN, HR_DEPTH_MAX, (clipZ / clipW + 1) / 2)
        U32 zy;
        U32 zb;
        U32 id; // Triangle id.
    };
    ...
    struct HRParams
    {
        ...
        HRAtomics *atomics; // Work counters. Per-image.
        ...
        S32 heightPixelsVp;
        S32 widthBins;  // widthPixels / HR_BIN_SIZE
        S32 heightBins; // heightPixels / HR_BIN_SIZE
        ...
        S32 widthTiles;  // widthPixels / HR_TILE_SIZE
        S32 heightTiles; // heightPixels / HR_TILE_SIZE
        ...
        void *triHeader;  // maxSubtris * HRTriangleHeader
        void *triData;    // maxSubtris * HRTriangleData

        // Bin output / coarse input.

        void *binSegData;  // maxBinSegs * HR_BIN_SEG_SIZE * S32
        void *binSegNext;  // maxBinSegs * S32
        void *binSegCount; // maxBinSegs * S32
        void *binFirstSeg; // HR_MAXBINS_SQR * HR_BIN_STREAMS_SIZE * (S32 segIdx), -1 = none
        void *binTotal;    // HR_MAXBINS_SQR * HR_BIN_STREAMS_SIZE * (S32 numTris)

        // Coarse output / fine input.

        void *tileSegData;  // maxTileSegs * HR_TILE_SEG_SIZE * S32
        void *tileSegNext;  // maxTileSegs * S32
        void *tileSegCount; // maxTileSegs * S32
        void *activeTiles;  // HR_MAXTILES_SQR * (S32 tileIdx)
        void *tileFirstSeg; // HR_MAXTILES_SQR * (S32 segIdx), -1 = none
        ...
        HRImageParams imageParamsFirst[HR_EMBED_IMAGE_PARAMS];
        const HRImageParams *imageParamsExtra; // After HR_EMBED_IMAGE_PARAMS.
    };
