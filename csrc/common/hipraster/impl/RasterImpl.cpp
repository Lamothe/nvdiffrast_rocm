    ...
    m_maxTileSegs           (1)
{
    ...
    NVDR_CHECK_CUDA_ERROR(hipOccupancyMaxActiveBlocksPerMultiprocessor(&m_numCoarseBlocksPerSM, (void*)coarseRasterKernel, 32 * CR_COARSE_WARPS, 0));
...
void RasterImpl::launchStages(bool instanceMode, bool peel, hipStream_t stream)
{
    ...
    NVDR_CHECK_CUDA_ERROR(hipMemcpyAsync(m_crAtomics.getPtr(), atomics, m_numImages * sizeof(CRAtomics), hipMemcpyHostToDevice, stream));
    ...
    if (!peel && m_numImages > CR_EMBED_IMAGE_PARAMS)
    {
        ...
        NVDR_CHECK_CUDA_ERROR(hipMemcpyAsync(m_crImageParamsExtra.getPtr(), imageParams + CR_EMBED_IMAGE_PARAMS, numImageParamsExtra * sizeof(CRImageParams), hipMemcpyHostToDevice, stream));
    }
    ...
    if (!peel)
    {
        if (instanceMode)
        {
            ...
            NVDR_CHECK_CUDA_ERROR(hipLaunchKernel((void*)triangleSetupKernel, dim3(setupBlocks, 1, m_numImages), dim3(32, CR_SETUP_WARPS), args, 0, stream));
        }
        else
        {
            for (int i=0; i < m_numImages; i++)
            ...
            NVDR_CHECK_CUDA_ERROR(hipLaunchKernel((void*)triangleSetupKernel, dim3(setupBlocks, 1, 1), dim3(32, CR_SETUP_WARPS), args, 0, stream));
        }
        NVDR_CHECK_CUDA_ERROR(hipLaunchKernel((void*)binRasterKernel, dim3(CR_BIN_STREAMS_SIZE, 1, m_numImages), brBlock, args, 0, stream));
        ...
        NVDR_CHECK_CUDA_ERROR(hipMemcpyAsync(m_crAtomicsHost.getPtr(), m_crAtomics.getPtr(), sizeof(CRAtomics) * m_numImages, hipMemcpyDeviceToHost, stream));
    }
