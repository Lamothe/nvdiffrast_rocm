...
// HipRaster codebase that accompanied the HPG 2011 paper
...
namespace HR
{
    ...
    public:
        ...
        bool drawTriangles(const int *ranges, bool peel, hipStream_t stream); // Ranges (offsets and counts) as #triangles entries, not as bytes. If NULL, draw all triangles. Returns false in case of internal overflow.
        void *getColorBuffer(void);                                           // GPU pointer managed by HipRaster.
        void *getDepthBuffer(void);                                           // GPU pointer managed by HipRaster.
        void swapDepthAndPeel(void);                                          // Swap depth and peeling buffers.

    private:
        HipRaster(const HipRaster &);            // forbidden
        HipRaster &operator=(const HipRaster &); // forbidden
