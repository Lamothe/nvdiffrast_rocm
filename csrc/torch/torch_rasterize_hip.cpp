...
#include "../common/hipraster/HipRaster.hpp"
#include "../common/hipraster/impl/Constants.hpp"
#include <tuple>
#include <c10/hip/HIPStream.h>
#include <c10/core/OptionalDeviceGuard.h>
...
//------------------------------------------------------------------------
// Python HipRaster state wrapper methods.

RasterizeCRStateWrapper::RasterizeCRStateWrapper(int cudaDeviceIdx_)
{
    const c10::OptionalDeviceGuard device_guard(cudaDeviceIdx_);
...
{
    const c10::OptionalDeviceGuard device_guard(cudaDeviceIdx);
    delete cr;
}
...
{
    const c10::OptionalDeviceGuard device_guard(device_of(pos));
    hipStream_t stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA();
    HR::HipRaster *cr = stateWrapper.cr;
    ...
    // Check that HipRaster context was created for the correct GPU.
    NVDR_CHECK(pos.get_device() == stateWrapper.cudaDeviceIdx, "HipRaster context must must reside on the same device as input tensors");
    ...
    int height = (height_out + HR_TILE_SIZE - 1) & (-HR_TILE_SIZE);
    int width = (width_out + HR_TILE_SIZE - 1) & (-HR_TILE_SIZE);
    ...
    cr->setRenderModeFlags(enablePeel ? HR::HipRaster::RenderModeFlag_EnableDepthPeeling : 0); // No backface culling.
    if (enablePeel)
        cr->swapDepthAndPeel(); // Use previous depth buffer as peeling depth input.

    // Determine viewport tiling.
    int tileCountX = (width + HR_MAXVIEWPORT_SIZE - 1) / HR_MAXVIEWPORT_SIZE;
    int tileCountY = (height + HR_MAXVIEWPORT_SIZE - 1) / HR_MAXVIEWPORT_SIZE;
    int tileSizeX = ((width + tileCountX - 1) / tileCountX + HR_TILE_SIZE - 1) & (-HR_TILE_SIZE);
    int tileSizeY = ((height + tileCountY - 1) / tileCountY + HR_TILE_SIZE - 1) & (-HR_TILE_SIZE);
    ...
    TORCH_CHECK(tileSizeX <= HR_MAXVIEWPORT_SIZE && tileSizeY <= HR_MAXVIEWPORT_SIZE, "internal error in tile size calculation: tile larger than allowed");
    TORCH_CHECK((tileSizeX & (HR_TILE_SIZE - 1)) == 0 && (tileSizeY & (HR_TILE_SIZE - 1)) == 0, "internal error in tile size calculation: tile not divisible by ", HR_TILE_SIZE);
    ...
    for (int tileY = 0; tileY < tileCountY; tileY++)
        ...
        {
            // Set HipRaster viewport according to tile.
...
{
    const c10::OptionalHIPGuardMasqueradingAsCUDA device_guard(device_of(pos));
    hipStream_t stream = at::hip::getCurrentHIPStreamMasqueradingAsCUDA();
